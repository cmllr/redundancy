...
<input type= "checkbox" name="User_NoLogout_Warning" <?php
	if ($GLOBALS["config"]["User_NoLogout_Warning"] == 1)
		echo "checked=\"true\"";	
?>> <?php echo $GLOBALS["Program_Language"]["User_NoLogout_Warning"];?><p>
<input type= "checkbox" name="Program_Display_Icons_if_needed"<?php
	if ($GLOBALS["config"]["Program_Display_Icons_if_needed"] == 1)
	echo "checked=\"true\"";	
?>> <?php echo $GLOBALS["Program_Language"]["Program_Display_Icons_if_needed"];?><p>
<input type= "checkbox" name="Program_Enable_JQuery"<?php
	if ($GLOBALS["config"]["Program_Enable_JQuery"] == 1)
		echo "checked=\"true\"";	
?>> <?php echo $GLOBALS["Program_Language"]["Program_Enable_JQuery"];?><p>
<input type= "checkbox" name="Program_Enable_Preview" <?php
	if ($GLOBALS["config"]["Program_Enable_Preview"] == 1)
		echo "checked=\"true\"";	
?>> <?php echo $GLOBALS["Program_Language"]["Program_Enable_Preview"];?><p>
 <input type="submit" value="<?php echo $GLOBALS["Program_Language"]["Save"]; ?>" /> 
</form>
<form method="POST" action="index.php?module=setpass" >
<input type="submit" value="<?php echo $GLOBALS["Program_Language"]["New_Pass"]; ?>" />
</form>